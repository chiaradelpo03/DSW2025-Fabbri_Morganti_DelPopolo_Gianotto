...
const createOrder = async (req, res) => {
  try {
    ...
    // Obtener todos los productos del carrito de una sola vez
    const productIds = items.map(item => item.productId);
    const productosDB = await Product.findAll({ where: { id: productIds } });

    // Mapear los productos a sus precios
    const preciosMap = {};
    productosDB.forEach(product => {
      preciosMap[product.id] = product.price;
    });

    // Crear el array de productos para OrderProducts con precio incluido
    const orderProductItems = items.map(item => ({
      orderId: order.id,
      productId: item.productId,
      quantity: item.quantity,
      price_at_purchase: preciosMap[item.productId] || 0,
    }));
